frappe.ui.form.on('Pick List', {
    ...
    validate:function(frm){
        if(frm.doc.custom_bo_current_box_type){
            frappe.throw("Close the current box value.By clicking the 'Close Box' button.")
        }        
    },
    custom_bo_select_box:function(frm){
        if(frm.doc.custom_current_box_type){
            frappe.throw("Please Close Current box to select new box.")
        }
        if(frm.doc.custom_bo_scan_barcodes){
            frm.set_value("custom_bo_scan_barcodes","")
            frm.refresh_field("custom_bo_scan_barcodes")
        }
        let d = new frappe.ui.Dialog({
            ...
            primary_action(values) {
                frm.set_value("custom_bo_current_box_type",values.box_type)
                frm.refresh_field("custom_bo_current_box_type")
                d.hide();
            }
        });
    ...
    custom_bo_close_box(frm) {
        ...
        for (const location of frm.doc.locations) {
            if (location.qty !== location.picked_qty && location.picked_qty !== 0) {
                let remaining_qty = location.qty - location.picked_qty;
    
                // Update the current location with picked_qty
                location.qty = location.picked_qty;
                location.stock_qty = location.picked_qty;
                location.custom_bo_box_count = newCount;
                location.idx = idx;
                idx++;
    
                // Add the updated location to the new locations array
                new_locations.push(location);
    
                // Create a new location for the remaining qty
                new_locations.push({
                ...
                // Update the current location's custom_box_count
                location.idx = idx;
                idx++;
                new_locations.push(location);
    
                // If picked_qty is the same as qty, still mark locationsUpdated as true
                if (location.qty === location.picked_qty) {
                    locationsUpdated = true;
                }
            }
        }
        ...
        if (locationsUpdated) {
            frm.doc.locations = [];
            for (const element of new_locations) {
                frm.add_child("locations", element);
            }
            frm.refresh_field("locations");
    
            frm.set_value("pick_manually", true);
            frm.refresh_field("pick_manually");
    
            frm.set_value("custom_bo_current_box_type", "");
            frm.refresh_fields(["custom_bo_current_box_type", "locations"]);
            frm.save();
        }

        frappe.flags.active_row = null;
    },

    custom_bo_clear_box:function(frm){
        
        let currentBoxCount = frm.doc.custom_bo_current_box_count || 0;

        let d = new frappe.ui.Dialog({
            title: 'Box Count details',
            fields: [
                {
                    label: 'Box Count',
                    fieldname: 'custom_current_box_count',
                    fieldtype: 'Int',
                    default: currentBoxCount
                },
            ],
            size: 'small',
            primary_action_label: 'Submit',
            primary_action(values) {
                frm.doc.locations.forEach(element => {
                    if(frm.doc.custom_bo_current_box_count === element.custom_bo_box_count){
                        element.custom_bo_box_count = 0
                        element.picked_qty = 0
                        element.custom_bo_box_type = ""
                    }
                });
                frm.refresh_field("locations")
                let new_value = frm.doc.custom_bo_current_box_count - 1

                frm.set_value("custom_bo_current_box_count",new_value)
                frm.refresh_field("custom_bo_current_box_count")
                frm.save();
                d.hide();
            }
    ...
    custom_bo_scan_barcodes: function(frm) {
        class CustomBoBarcodeScanners  {
            constructor(opts) {
                ...
                this.scan_field_name = opts.scan_field_name || "custom_bo_scan_barcodes";
                this.scan_barcode_field = this.frm.fields_dict[this.scan_field_name];

                this.barcode_field = opts.barcode_field || "custom_barcode";
            ...
            update_table(data) {
                return new Promise((resolve, reject) => {
                    ...
                    frappe.run_serially([
                        ...
                        () => this.clean_up(),
                        () => this.set_custom_box_type(row, frm.doc.custom_bo_current_box_type),
            ...
            async set_custom_box_type(row, custom_box_type) {
                frappe.flags.active_row = row.idx;
                if (custom_box_type) {
                    await frappe.model.set_value(row.doctype, row.name, "custom_bo_box_type", custom_box_type);
                }
        ...
        const opts = {
            frm: frm,
            scan_field_name:"custom_bo_scan_barcodes",
