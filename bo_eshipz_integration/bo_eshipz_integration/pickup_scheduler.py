...
def schedule_update_delivery_date_for_pf(start=0):
    try:
        ...
        frappe.log_error(
            "Error Updating Pickup Forms of Delivery Date", frappe.get_traceback()
        )


def get_delivery_date_from_shipping_details(pdf_name):
    """
    Retrieve delivery / expected delivery date from tracking API
    and update Pickup Forms.
    """
    try:
        ...
        if not isinstance(tracking_data, list) or not tracking_data:
            frappe.log_error(
                title="get_delivery_date_from_shipping_details for Pickup Forms",
                message=f"Eshipz Shipping Delivery Date for Pickup Forms {pdf_name} Not Found",
            )
            return
        ...
        if delivery_date_str:
            ...
            update_values["actual_delivery_date"] = delivery_date.strftime("%Y-%m-%d")
            update_values["eshipz_shipment_status"] = "Delivered"
        ...
        # ✅ Expected delivery date (InTransit case)
            ...
            update_values["expected_delivery_date"] = expected_delivery_date.strftime(
                "%Y-%m-%d"
            )

        # 🔹 Update only if we have something to save
        if update_values:
            frappe.db.set_value("Pickup Forms", pdf_name, update_values)

    except Exception:
        frappe.log_error(
            title=f"Pickup Forms Delivery Date Update Failed: {pdf_name}",
...
def schedule_update_shipping_detail_status_for_pf(start=0):
    try:
        ...
        if not pickup_dispatch_forms:
            frappe.log_error(
                "Batch Processing Complete", "No more Pickup Forms left to process."
            )
            return
        ...
        if updated_pickup_dispatch_forms:
            frappe.log_error(
                "Updated Pickup Forms For Detail Status",
        ...
        frappe.log_error(
            "Error While Update Shipment Status From Scheduler For Pickup Forms",
            message=frappe.get_traceback(),
        )
...
def get_shipping_details_status(pdf_name):
    try:
        ...
        if not isinstance(tracking_data, list) or not tracking_data:
            frappe.log_error(
                title="get_shipping_details_status for Pickup Forms",
                message=f"Shipping Detail Status for Pickup Forms {pdf_name} Not Found",
            )
            return
        ...
        existing_status = frappe.db.get_value(
            "Pickup Forms", pdf_name, "eshipz_shipment_status"
        )

        # 🚫 Cancelled shipment → ONLY update Cancelled & exit
        if shipment_status == "cancelled":
            frappe.db.set_value(
                "Pickup Forms", pdf_name, "eshipz_shipment_status", "Cancelled"
            )
        ...
        if existing_status != "Delivered" and tag_status:
            frappe.db.set_value(
                "Pickup Forms", pdf_name, "eshipz_shipment_status", tag_status
            )
            return True, pdf_name

    except Exception:
        frappe.log_error(
            title=f"Error getting shipping details status for Pickup Forms {pdf_name}",
            message=frappe.get_traceback(),
        )
